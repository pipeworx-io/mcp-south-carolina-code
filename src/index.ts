...
/**
 * South Carolina Code of Laws — state statutes by citation.
 *
 * Files are per CHAPTER and citations are per section, as in Texas: the page
 * for "16-3-10" is t16c003.php, which carries the whole of title 16 chapter 3
 * — 331,669 characters covering dozens of sections. Returning that for a
 * one-section request would bury the answer, so the section is cut out,
 * bounded by the next "SECTION 16-3-NN." heading.
 *
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'South Carolina Code of Laws');
}

const BASE = 'https://www.scstatehouse.gov/code';
const UA = 'pipeworx-mcp-south-carolina-code/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [{
  name: 'sc_code_section',
  description:
  ...
  inputSchema: {
    type: 'object',
    properties: {
      section: { type: 'string', description: 'Section as cited, e.g. "16-3-10" (murder) or "56-5-2930" (DUI). Title-chapter-section.' },
    },
    required: ['section'],
  },
}];
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  if (name !== 'sc_code_section') throw new Error(`Unknown tool: ${name}`);
  const raw = String(args.section ?? '').trim().replace(/\s+/g, '');
  const m = /^(\d+)-(\d+)-(\d+)$/.exec(raw);
  if (!m) {
    return { found: false, reason: 'unparsed_section',
      message: `"${args.section}" is not a South Carolina citation.`,
      hint: 'SC sections are title-chapter-section, e.g. "16-3-10" (murder) or "56-5-2930" (DUI).' };
  }
  const [, title, chapter] = m;
  const url = `${BASE}/t${title.padStart(2, '0')}c${chapter.padStart(3, '0')}.php`;
  const res = await pwFetch(url, { headers: { 'User-Agent': UA } });
  if (res.status === 404) {
    return { found: false, reason: 'chapter_not_found', section: raw, title, chapter,
      message: `No title ${title} chapter ${chapter} in the South Carolina Code (HTTP 404).`,
      hint: 'Chapter numbers are not contiguous — repealed chapters are normal.' };
  }
  if (!res.ok) throw new Error(`South Carolina Statehouse returned HTTP ${res.status} for ${raw}`);

  const text = statuteText(await res.text());
  const esc = raw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const start = text.search(new RegExp(`SECTION\\s+${esc}\\.`, 'i'));
  if (start === -1) {
    return { found: false, reason: 'section_not_found', section: raw, title, chapter,
      message: `Title ${title} chapter ${chapter} exists but has no section ${raw}.`,
      hint: 'Section numbers are not contiguous — repealed sections are normal.' };
  }
  const rest = text.slice(start + 8);
  const nextRel = rest.search(/SECTION\s+\d+-\d+-\d+\./i);
  const body = (nextRel === -1 ? text.slice(start) : text.slice(start, start + 8 + nextRel)).trim();

  return {
    citation: `S.C. Code § ${raw}`, section: raw, title, chapter,
    text: body, text_chars: body.length, url,
    source: 'South Carolina Legislature (scstatehouse.gov), keyless',
    attribution: 'South Carolina statutes are public record.',
  };
}
